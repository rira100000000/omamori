...
RSpec.describe Omamori::CoreRunner do
  ...
  before do
    ...
    allow(config_double).to receive(:get).with('checks',
                                               any_args).and_return(Omamori::CoreRunner::DEFAULT_RISKS_TO_CHECK)
    ...
    context 'when the scan command is specified' do
      ...
      context 'with --diff option' do
        ...
        it 'calls diff_splitter if the diff is large' do
          ...
          expect(diff_splitter_double).to receive(:process_in_chunks).with(large_diff, anything, anything, anything,
      ...
      context 'with --all option' do
        ...
        it 'calls diff_splitter if the codebase is large' do
          ...
          expect(runner).to receive(:get_full_codebase)
          expect(gemini_client_double).not_to receive(:analyze)
          expect(diff_splitter_double).to receive(:process_in_chunks).with(large_codebase, anything, anything,
                                                                           anything, anything, model: anything)
      ...
      context 'with --format option' do
        let(:results) do
          { 'security_risks' => [{ 'risk' => 'SQL Injection', 'severity' => 'High', 'message' => 'Example' }] }
        end
      ...
      context 'when scan command is specified with paths' do
        ...
        let(:combined_content) do
          "# File: /project/root/file1.rb\n#{file1_content}\n\n# File: /project/root/dir/file2.rb\n#{file2_content}\n\n"
        end
        ...
        it 'calls collect_files_from_paths and analyzes each file' do
          ...
          allow(runner).to receive(:parse_options).and_wrap_original do |method, *args|
            ...
            runner.instance_variable_set(:@options,
                                         { command: :scan, scan_mode: :paths, format: :console,
                                           force_scan_ignored: force_scan_ignored })
          ...
          expect(prompt_manager_double).to receive(:build_prompt).with(file1_content, anything, anything,
                                                                       file_path: '/project/root/file1.rb')
          expect(prompt_manager_double).to receive(:build_prompt).with(file2_content, anything, anything,
                                                                       file_path: '/project/root/dir/file2.rb')
        ...
        it 'uses diff_splitter for large files' do
          ...
          expect(diff_splitter_double).to receive(:process_in_chunks).with(large_file_content, anything, anything,
                                                                           anything, anything, file_path: '/project/root/large_file.rb', model: anything)
        ...
        context 'and .omamoriignore is present' do
          ...
          it 'excludes files matching ignore patterns' do
            ...
            allow(runner).to receive(:parse_options).and_wrap_original do |method, *args|
              ...
              runner.instance_variable_set(:@options,
                                           { command: :scan, scan_mode: :paths, format: :console,
                                             force_scan_ignored: false })
              runner.instance_variable_set(:@target_paths, ['file1.rb', 'ignored_file.rb', 'ignored_dir/'])
            end

            expect(runner).to receive(:collect_files_from_paths).with(['file1.rb', 'ignored_file.rb', 'ignored_dir/'],
                                                                      anything, false).and_return(expected_files_after_ignore)
          ...
          context 'and --force-scan-ignored is true' do
            ...
            it 'includes files matching ignore patterns' do
              ...
              expected_files_after_ignore = ['/project/root/file1.rb', '/project/root/ignored_file.rb',
                                             '/project/root/ignored_dir/file3.rb']
      ...
      context 'when the scan command is specified with --ai option' do
        it 'runs only AI analysis, skipping static analysers' do
          ...
          allow(runner).to receive(:parse_options).and_wrap_original do |method, *args|
            method.call(*args) # Call original parse_options
            runner.instance_variable_set(:@options,
                                         { command: :scan, scan_mode: :diff, format: :console, only_ai: true })
          end
    ...
    describe '#collect_files_from_paths' do
      ...
      context 'when given a file path' do
        it "returns the file path if it's a Ruby file and not ignored" do
          ...
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/file.rb'), ignore_patterns,
                                                                  force_scan_ignored).and_return(false)
          expect(runner.send(:collect_files_from_paths, ['path/to/file.rb'], ignore_patterns,
                             force_scan_ignored)).to eq([File.expand_path('path/to/file.rb')])
        end
        ...
        it "does not return the file path if it's not a Ruby file" do
          ...
          expect(runner.send(:collect_files_from_paths, ['path/to/file.txt'], ignore_patterns,
                             force_scan_ignored)).to be_empty
        end
        ...
        it 'does not return the file path if it is ignored' do
          ...
          allow(runner).to receive(:matches_ignore_pattern?).with('path/to/ignored.rb', ignore_patterns,
                                                                  force_scan_ignored).and_return(true)
          expect(runner.send(:collect_files_from_paths, ['path/to/ignored.rb'], ignore_patterns,
                             force_scan_ignored)).to be_empty
        end
        ...
        it 'returns the file path if it is ignored but force_scan_ignored is true' do
          ...
          expect(runner.send(:collect_files_from_paths, ['path/to/ignored.rb'], ignore_patterns,
                             true)).to eq([File.expand_path('path/to/ignored.rb')])
        end
      end
      ...
      context 'when given a directory path' do
        it 'returns all Ruby files within the directory recursively that are not ignored' do
          ...
          allow(Dir).to receive(:glob).with(File.join(File.expand_path('path/to/dir'), '**',
                                                      '*.rb')).and_return(['path/to/dir/file1.rb',
                                                                           'path/to/dir/subdir/file2.rb'].map { |f|
                                                                            File.expand_path(f)
                                                                          })
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/file1.rb'),
                                                                  ignore_patterns, force_scan_ignored).and_return(false)
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/subdir/file2.rb'),
                                                                  ignore_patterns, force_scan_ignored).and_return(false)
          ...
          expect(runner.send(:collect_files_from_paths, ['path/to/dir'], ignore_patterns,
                             force_scan_ignored)).to match_array(expected_files)
        end
        ...
        it 'does not return ignored files within the directory' do
          ...
          allow(Dir).to receive(:glob).with(File.join(File.expand_path('path/to/dir'), '**',
                                                      '*.rb')).and_return(['path/to/dir/file1.rb',
                                                                           'path/to/dir/ignored_file.rb'].map { |f|
                                                                            File.expand_path(f)
                                                                          })
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/file1.rb'),
                                                                  ignore_patterns, force_scan_ignored).and_return(false)
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/ignored_file.rb'),
                                                                  ignore_patterns, force_scan_ignored).and_return(true)
        ...
        it 'returns ignored files within the directory if force_scan_ignored is true' do
          ...
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/file1.rb'),
                                                                  ignore_patterns, true).and_return(false)
          ...
          expect(runner.send(:collect_files_from_paths, ['path/to/dir'], ignore_patterns,
                             true)).to match_array(expected_files)
        end
      end
      ...
      context 'when given multiple paths' do
        it 'returns unique Ruby files from all specified paths' do
          ...
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('file1.rb'), ignore_patterns,
                                                                  force_scan_ignored).and_return(false)

          allow(File).to receive(:directory?).with(File.expand_path('path/to/dir')).and_return(true)
          allow(Dir).to receive(:glob).with(File.join(File.expand_path('path/to/dir'), '**',
                                                      '*.rb')).and_return(['path/to/dir/file2.rb'].map { |f|
                                                                            File.expand_path(f)
                                                                          })
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/file2.rb'),
                                                                  ignore_patterns, force_scan_ignored).and_return(false)
          allow(runner).to receive(:matches_ignore_pattern?).with(File.expand_path('path/to/dir/file1.rb'),
                                                                  ignore_patterns, force_scan_ignored).and_return(false)

          expected_files = [File.expand_path('file1.rb'), File.expand_path('path/to/dir/file2.rb')]
          expect(runner.send(:collect_files_from_paths, ['file1.rb', 'path/to/dir'], ignore_patterns,
                             force_scan_ignored)).to match_array(expected_files)
        end
      end
      ...
      context 'when given a non-existent path' do
        it 'prints a warning and does not return any files' do
          ...
          expect do
            runner.send(:collect_files_from_paths, ['non_existent_path'], ignore_patterns,
                        force_scan_ignored)
          end.to output(%r{Warning: Path not found or is not a file/directory: non_existent_path}).to_stdout
          expect(runner.send(:collect_files_from_paths, ['non_existent_path'], ignore_patterns,
                             force_scan_ignored)).to be_empty
        end
      end
    end
