...
RSpec.describe Omamori::AIAnalysisEngine::DiffSplitter do
  ...
  describe '#process_in_chunks' do
    ...
    before do
      allow(prompt_manager_double).to receive(:build_prompt).with(chunk1, risks_to_check,
                                                                  json_schema).and_return(chunk1_prompt)
      allow(prompt_manager_double).to receive(:build_prompt).with(chunk2, risks_to_check,
                                                                  json_schema).and_return(chunk2_prompt)
      allow(prompt_manager_double).to receive(:build_prompt).with(chunk3, risks_to_check,
                                                                  json_schema).and_return(chunk3_prompt)

      allow(gemini_client_double).to receive(:analyze).with(chunk1_prompt, json_schema,
                                                            model: 'gemini-1.5-pro-latest').and_return(chunk1_result)
      allow(gemini_client_double).to receive(:analyze).with(chunk2_prompt, json_schema,
                                                            model: 'gemini-1.5-pro-latest').and_return(chunk2_result)
      allow(gemini_client_double).to receive(:analyze).with(chunk3_prompt, json_schema,
                                                            model: 'gemini-1.5-pro-latest').and_return(chunk3_result)
    ...
    it 'handles content smaller than chunk size without splitting' do
      ...
      allow(prompt_manager_double).to receive(:build_prompt).with(short_content, risks_to_check,
                                                                  json_schema).and_return(short_prompt)
      allow(gemini_client_double).to receive(:analyze).with(short_prompt, json_schema,
                                                            model: 'gemini-1.5-pro-latest').and_return(short_result)
    ...
    it 'handles nil results from analyze' do
      allow(prompt_manager_double).to receive(:build_prompt).with(anything, anything,
                                                                  json_schema).and_return('dummy prompt')
